...
- 宝箱：常用稀有物品的数量、最近距离、箱型、阶段锁和 8×8 降采样估算获取成本。
...
默认并发数为 1，适合边做别的事边后台生成。提高并发会显著增加 CPU、内存和磁盘压力，也可能令游戏本身卡顿。
...
- 获取成本是一次 8×8 降采样 Dijkstra 的相对分数，考虑空气、挖掘、邪恶/地牢/神庙硬度、水、岩浆和锁箱；它不假装模拟玩家的装备、炸药或操作路线。
